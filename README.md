...
1️⃣ Phase 1: Single Model Analysis (<1차_Single model>)  
단일 모델을 통해 변수 간 상관관계를 파악하고 추천의 기초가 되는 확률 분포를 분석했습니다.  

Step1_확률분포 & results: 곡 특징별 분포 및 기초 분석 결과  

correlation_results: 변수 간 상관관계 분석  

feature_importance_FINAL: 추천 모델의 핵심 변수 중요도 산출  

step1+2+3.R: 추천 알고리즘 단계별 통합 구현 코드  

playlist_plots_FINAL: 생성된 플레이리스트 시각화 결과물  

2️⃣ Phase 2: Multi Model & Optimization (<2차_Multi model>)  
다양한 지표를 도입하고 실험을 통해 모델을 고도화했습니다.  

Baseline Study:  
baseline(vectorized).R: 벡터화된 기준 모델 구현  
plot_context_correlation_heatmap.pdf: 컨텍스트 간 상관관계 분석 히트맵  
